...
class TransalliancePdfAssistant extends PdfClient
{
        ...
        foreach ($loading_locations_lines as $i => $linesIndex) {
            ...
            if ($timerange_line) {
                ...
                $datetime_from = $carbonDate->startOfDay()
                    ->addHours($this->parseHourString($times[0]))
                    ->toISOString();

                $datetime_to = $carbonDate->startOfDay()
                    ->addHours($this->parseHourString($times[1]))
                    ->toISOString();
            }
    ...
    protected function parseHourString(string $hourString): int
    {
        $hourString = $this->parseHour($hourString);
